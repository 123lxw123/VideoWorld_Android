...
import com.noahwm.king.framework.downloadmanager.DownloadStatusListener;
import com.noahwm.king.framework.network.HttpConnecter;
import com.noahwm.king.framework.util.FileSystemUtil;
import com.noahwm.king.framework.util.ToastUtil;
import com.noahwm.king.framework.widget.CustomConfirmDialog;
...
public class UpdateAppDialog extends AlertDialog {
	...
	public UpdateAppDialog(Activity context) {
		super(context);
		this.context=context;
	}

	public UpdateAppDialog(Activity context, int theme) {
		super(context, theme);
		this.context=context;
	}

	public UpdateAppDialog(Activity context, String head, String content,
                           String downUrl, boolean forceClose, View.OnClickListener listener) {
		super(context);
		this.context = context;
		mHeadStr = head;
		mContentStr = content;
		...
		update_version_dialog_ok_btn
				.setOnClickListener(new View.OnClickListener() {
					...
					public void onClick(View v) {
						// 下载更新 mDownUrl
						mHandler.sendMessage(mHandler.obtainMessage(DOWNLOAD_APK));
					}
				});
		update_version_dialog_head_tev = (TextView) this.findViewById(R.id.update_version_dialog_head_tev);
		update_version_dialog_head_tev.setText(mHeadStr);
		update_version_dialog_content_edt = (EditText) this.findViewById(R.id.update_version_dialog_content_edt);
...
//									UpdateAppDialog.this.getContext());
//						}
//					});
		} else {
			update_version_dialog_cancel_btn.setText("稍后下载");
...
//					});
		...
		update_version_dialog_cancel_btn.setOnClickListener(mListener);
		...
		public void handleMessage(Message msg) {
			...
			case DOWNLOAD_APK:
				...
				updateFile = FileSystemUtil.createFilePath(
						UpdateAppDialog.this.getOwnerActivity(), "update",
				...
				//隐藏是否更新对话框，弹出下载进度对话框
				UpdateAppDialog.this.dismiss();
				downLoadProgressDialog=new DownLoadProgressDialog(UpdateAppDialog.this.getContext(),UpdateAppDialog.this);
				...
				UpdateAppDialog.this.setCancelable(false);
				UpdateAppDialog.this.setCanceledOnTouchOutside(false);
			...
			case DOWNLOAD_SUCCESS:
				...
				UpdateAppDialog.this.getContext().startActivity(
						installIntent);
			...
			case DOWNLOAD_FAILURE:
				downLoadProgressDialog.dismiss();
				updateNotificationManager.cancel(0);
				ToastUtil.showMessage(UpdateAppDialog.this.getContext(),
